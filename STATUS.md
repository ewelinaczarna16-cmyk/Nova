...
- Tier 5 done, 2026-07-05 — heartbeat loop: config-driven checks, restart-safe schedule, quiet hours + held/dismissible notices, SMS channel (dry-run until Twilio wired), gated background actions. Daily digest proves the channel via `--digest-now`. Ticks the "daily digest lands as a text" v1 bar (pending real Twilio creds).
